...
public class SlidingPuzzle {
	...
	public static final int[] TILE_1 = { 12125, 12133, 12141, 12149 }, TILE_2 = { 12126,
			12134, 12142, 12150 }, TILE_3 = { 12127, 12135, 12143, 12151 };
	public static final int[] TILE_4 = { 12128, 12136, 12144, 12152 }, TILE_5 = { 12129,
			12137, 12145, 12153 }, TILE_6 = { 12130, 12138, 12146, 12154 };
	public static final int[] TILE_7 = { 12131, 12139, 12147, 12155 }, TILE_8 = { 12132,
			12140, 12148, 12156 }, TILE_9 = null;
	...
	public static void solve(){
		...
		for (int i = 0; i < ID_SPOTS.length; i++) {
			for (int j = 0; j < ID_SPOTS[i].length; j++) {
                ...
                Tile [] Tiles = new Tile[]{t1.getLocation(),new Tile(dx,dy -1,0),new Tile(dx -1 ,dy,0),new Tile(dx -1 ,dy -1,0)};
				for (Tile t2 : Tiles) {
					for (Tile t3 : EMPTY_AREA) {
						if (Calculations.distance(t2, t3) == 1) {
							if(!t1.isOnScreen()){
								Walking.walk(t1);
								Camera.turnTo(t1);
								for(int sl = 0 ; sl < 40 && Players.getLocal().isMoving(); sl++,Task.sleep(100,150));
							}
							t1.interact("Move");
							Task.sleep(1000,1500);
							for(int sl = 0 ; sl < 40 && t1.isMoving(); sl++,Task.sleep(100,150));
						}
					}
				}
			}
		}
		if(SetupTest.Message.toLowerCase().contains("you hear a click") ||SetupTest.Message.toLowerCase().contains("unlocks") ){
			Solved = true;	
		}
	}
